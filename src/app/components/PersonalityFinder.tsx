      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-20 text-center md:text-left flex flex-col md:flex-row justify-between items-end">
          <div className="max-w-xl">
            <h2 className="font-serif text-4xl md:text-5xl text-brand mb-6 tracking-[0.05em]">
              Find Your Archetype
            </h2>
            <p className="font-sans text-secondary leading-relaxed tracking-wide">
              We transcend generic filters. Discover the scent profile that matches the energy you wish to project.
            </p>
          </div>
          <button className="hidden md:flex items-center gap-2 uppercase tracking-widest text-accent font-sans font-semibold border-b border-accent pb-1 hover:text-brand hover:border-brand transition-colors">
        ...
        <div className="grid grid-cols-12 gap-8">
          {ARCHETYPES.map((item, index) => (
            ...
            >
              ...
              />
              <div className="absolute inset-0 bg-[var(--color-brand-dark)]/30 group-hover:bg-[var(--color-brand-dark)]/40 transition-colors duration-500" />
              
              <div className="absolute inset-0 p-10 flex flex-col justify-end">
                <span className="uppercase text-accent font-sans tracking-[0.2em] text-xs font-bold mb-3">
                  Archetype 0{index + 1}
                </span>
                <h3 className={`font-serif text-3xl md:text-4xl text-light mb-4 tracking-[0.05em]`}>
                  {item.title}
                </h3>
                <div className="overflow-hidden h-0 group-hover:h-20 transition-all duration-500 ease-in-out">
                  <p className="text-light/80 font-sans text-sm md:text-base max-w-sm mb-4">
                    {item.desc}
                  </p>
                  <button className="text-accent font-sans uppercase tracking-widest text-xs font-bold flex items-center gap-2 hover:text-light transition-colors">
